...
void ATTDevice::Subscribe(byte* mac, PubSubClient& mqttclient, char* clientKey)
{
...
void ATTDevice::MqttConnect()
{
	...
	while (!_mqttclient->connect(mqttId, (char*)brokerId.c_str(), _clientKey)) 
	{
		#ifdef DEBUG
...
{
	String MqttString = "client/" + _clientId + "/in/device/" + _deviceId + "/asset/+/command";   //arduinos are only intersted in actuator command, no management commands
