...
/*
  ...
  This file is an example sketch to deploy a digital Actuator in the SmartLiving.io IoT platform.
...
int DigitalActuator = 4;                                        // Digital Actuator is connected to pin D4 on grove shield 
...
{ 
  ...
  {	                                                      //put this in a sub block, so any unused memory can be freed as soon as possible, required to save mem while sending data
	...
	if (pinNr == DigitalActuator)  
	{
	  if (msgString == "false") {
            digitalWrite(DigitalActuator, LOW);		     //change the actuator status to false
            idOut = &DigitalActuator;		                        
	  }
	  else if (msgString == "true") {
	    digitalWrite(DigitalActuator, HIGH);              //change the actuator status to true
            idOut = &DigitalActuator;
	  }
	}
